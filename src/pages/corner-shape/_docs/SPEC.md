...
generator.astro        <form id="generator-form">
  ...
  shape-settings.astro   card properties, corner and axis modes, corner fields
...
4. `makeState()` — build `GeneratorState`: shapes, horizontal and vertical radii, the unit each corner was typed in, derived fallback radii, the measured card, selector, exact flag.
...
Custom properties driving the preview live in `_styles/vars.css` with their server-rendered starting values: `--size`, `--ratio`, `--p`, `--fs`, `--radius`, `--fb-radius`, `--shape-*`, `--preview-clip`.
...
| Radius mode      | Circular / Elliptical | —                                      | Circular                   |
| Preset           | 6-position toggle     | six keywords                           | Squircle                   |
| Superellipse `s` | slider + text         | `-3–5`, `0.1`                          | `2`                        |
| Radius ↔         | slider + number       | width ceiling per mode, step per unit  | `1rem`                     |
| Radius ↕         | slider + number       | height ceiling per mode, step per unit | `1rem`                     |
| Radius unit      | px / rem / %          | —                                      | rem                        |
| Preview tier     | Enhanced / Fallback   | —                                      | Enhanced                   |
| Exact fallback   | checkbox              | —                                      | off                        |
| Selector         | text                  | —                                      | `.card`                    |

Both corner modes stay in the markup and the mode switch shows one of them, so the form always serialises a complete answer for whichever mode is active. In Individual mode the preset, shape and radius fields repeat in a 2×2 grid mapped to the physical corners — shape and radius always travel together; there is no separate link toggle. Radius mode uses one set of fields. Circular hides the vertical row and mirrors the horizontal value into it; Elliptical exposes both. The axis arrow is a `.axis-glyph` span rather than part of the label string, so Circular can drop it and read plain `Radius` — with one number for both axes there is no axis to point at.
...
**Radius unit** — a three-position switch to the right of each pair of axis rows, and the only place a unit is chosen: the output block has no unit setting of its own. Both axes of one corner share it. The All-mode radius carries one switch, each of the four corners carries its own, and mixed-corner shorthand such as `border-radius: 40px 2rem 14% 1.2rem` remains valid.

A percentage is the one that is not just notation: it resolves against the width horizontally and the height vertically. Elliptical mode converts each axis against that axis's side. Circular mode still uses the shorter side because one visible number drives both axes.

**The ceiling is a fraction of that side, and the fraction depends on the mode.** Circular stops at half — `50%`, half the shorter side in px or rem — because one number moves both axes together, and past half the neighbours' overlap scales the corner straight back to where it was. Elliptical goes to the whole side — `100%`, the full width for ↔ and the full height for ↕ — because its axes are independent and `border-radius: 100% 0 0 0 / 50% 0 0 0` is a quarter-sweep no half-side value can draw. That upper half only shows in Individual mode: with all four corners equal, CSS scales them by one shared factor and `100%` lands back on the `50%` shape.
...
Flipping the switch converts both numbers so the corner keeps what it drew, through rem as the middle unit. px ↔ rem is exact. Elliptical percentages use width for ↔ and height for ↕, so their used lengths are exact. Circular percentages use the shorter side. Values land rounded to two decimals, so a round trip can drift by a hundredth.
...
A percentage reaches the clip path unconverted as well — percentages inside `polygon()` and `shape()` resolve against the width on one axis and the height on the other, exactly as `border-radius` does. Only the overlap clamp has to leave the corner's own unit: `clippedRadii` resolves the horizontal and vertical groups against their respective sides, `clampFactor` compares both groups against the card and returns one factor, and every radius takes that factor where it stands.
...
- Logical corner longhands are out of scope.
